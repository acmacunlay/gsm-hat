...
class GPS:
    ...
    def CalculateDeltaP(Position1, Position2):
        ...
        a = math.sin(deltaPhi / 2) * math.sin(deltaPhi / 2) + math.cos(phi1) * math.cos(
            phi2
        ...
        if self.__serData != "":
            if self.__readRAW > 0:
                ...
                if self.__readRAW == 1:
                    ...
                    if self.__serData == "OK\r\n":
                        ...
                        self.__smsToBuild.Message = (
                            self.__smsToBuild.Message + self.__serData
                        )
                ...
                if "ERROR" in self.__serData:
                    ...
                    self.__smsToBuild.Date = datetime.strptime(
                        rawData[3].replace('"', "")[:-3], "%y/%m/%d,%H:%M:%S"
                    )
                    self.__smsToBuild.Message = ""
                    ...
                    if len(rawData) == 3:
                        ...
                        if httpStatus == 200:  # Successful request
                            ...
                            self.__logger.info(
                                "HTTPACTION Network Error " + str(httpStatus)
                            )
                        else:
                            self.__logger.info(
                                "HTTPACTION Unhandled Error " + str(httpStatus)
                            )

                    else:
                        self.__logger.info(
                            "HTTPACTION return value is not expected: " + match[0][1]
                        )
                ...
                # unannounced data reception below (e.g. new SMS oder phone call)
                    ...
                    self.__logger.debug(
                        "New SMS in memory " + storage + " at position " + str(numSMS)
                    )
        ...
        if self.__sentTimeout > 0 and actTime > self.__sentTimeout:
            ...
            if self.__state == 2 or self.__state == 97:
                ...
                self.__logger.critical(
                    "Exception: Unhandled timeout during data reception"
                )
        ...
        while self.__working:
            ...
            if self.__state == 1:
                ...
                if self.__waitForUnlock():
                    retSMS = self.__smsSendList[0]
                    self.__logger.info(
                        "Message to " + retSMS.Receiver + " successfully sent"
                    )
                ...
                if self.__waitForUnlock():
                    if self.__sendToHat(
                        'AT+SAPBR=3,1,"APN","' + self.__GPRSuserAPN + '"'
                    ):
                        self.__state = self.__state + 1
                ...
                if self.__waitForUnlock():
                    if self.__sendToHat(
                        'AT+SAPBR=3,1,"USER","' + self.__GPRSuserUSER + '"'
                    ):
                        self.__state = self.__state + 1
                ...
                if self.__waitForUnlock():
                    if self.__sendToHat(
                        'AT+SAPBR=3,1,"PWD","' + self.__GPRSuserPWD + '"'
                    ):
                        self.__state = self.__state + 1
